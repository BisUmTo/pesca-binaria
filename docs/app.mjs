...
const colors=['#f4d754','#ffae52','#ec8de2','#67e3f8','#78a6ff','#be94ff','#efb0c6','#b5e67c','#83eee3','#f2be70','#d4b6ff'];
const hues=[190,170,100,0,40,70,125,235,295,180,65];
...
function preferences(){
 ...
 $('sound').textContent=prefs.sound?'♫':'♪';$('sound').setAttribute('aria-pressed',String(prefs.sound));$('sound').setAttribute('aria-label',prefs.sound?'Disattiva suoni':'Attiva suoni');
 $('motion').disabled=systemReduced;$('motion').setAttribute('aria-pressed',String(reduced));$('motion').setAttribute('aria-label',systemReduced?'Movimento ridotto dalle impostazioni del dispositivo':reduced?'Attiva animazioni':'Riduci animazioni');
...
function orb(value){const o=element('span','orb');o.append(element('span','orb-skin'),element('span','orb-number',value));return o;}
...
async function take(value,button){
 ...
 timer.pause();try{await flyMarble(source,$(`bit-orb-${value}`),colors[Math.log2(value)],`${hues[Math.log2(value)]}deg`);}finally{timer.resume();busy=false;render();}
 if(a.status==='won'){playSound('win');celebrate($('collector'));$('next').focus();}
 else if(a.status==='broken'){playSound('break');$('next').focus();}
 else [...$('balls').children].find(b=>!b.disabled)?.focus({preventScroll:true});
}
function render(){
 $('welcome').hidden=!!state;$('game').hidden=!state;if(!state)return;
 const a=state.current,s=summarize(state.attempts),done=a.status!=='playing',values=weightsFor(a),reading=a.mode==='read';
 ...
 for(const value of arranged(values,a.challengeId)){
  ...
  if(!reading){b.type='button';b.disabled=done||selected||busy;b.setAttribute('aria-label',`Pesca ${value}${selected?', già pescata':''}`);b.setAttribute('aria-pressed',String(selected));b.onclick=()=>take(value,b);}else b.setAttribute('aria-label',`${value}, bit ${selected?1:0}`);
  $('balls').append(b);
 }
 ...
 $('collector').className='collector'+(a.status==='broken'?' is-broken':a.status==='won'?' is-won':'');$('answer-form').hidden=!reading||done;$('answer').max=(a.rulesVersion??1)===1?511:2047;$('answer').value='';$('next').hidden=!done;$('next').textContent=a.status==='won'?'Prossima sfida  ▶':'Riprova questo numero';
...
$('download').onclick=async()=>{
 if(busy||!state?.attempts.length)return;$('download').disabled=true;
